...
public class Filme {
    ...
    public ArrayList<Filme> carregaFilmes(ArrayList<Filme> filmes){
        ...
        this.filmes.add(new Filme(6,1994,"O Máskara","Chuck Russell","Jim Carrey"));
        this.filmes.add(new Filme(7,2019,"It – Capítulo Dois","Andy Muschietti","Bill Skarsgård"));
    ...
    public void deletaFilme(ArrayList<Filme> filmes, int codigo){
        for (Filme f : filmes) {
            if (f.codigo == codigo){
                filmes.remove(f);
                System.out.println("\nFilme " + f.getNome() + " deletado com sucesso!\n");
                break;
            }
        }
    }
